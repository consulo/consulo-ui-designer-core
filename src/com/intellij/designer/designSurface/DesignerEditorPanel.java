...
public abstract class DesignerEditorPanel extends JPanel implements DataProvider, ModuleProvider, RadPropertyContext
{
	...
	public final void showError(@NotNull String message, @NotNull Throwable e)
	{
		...
		if(info.myShowLog)
		{
			LOG.error(LogMessageEx.createEvent(info.myDisplayMessage, info.myMessage + "\n" + ExceptionUtil.getThrowableText(info.myThrowable), AttachmentFactory.createAttachment(myFile)));
		}
	}
	...
	private class FixableMessageAction extends AbstractComboBoxAction<FixableMessageInfo>
	{
		...
		protected DefaultActionGroup createPopupActionGroup(JComponent button)
		{
			...
			{
				AnAction action;
				if((message.myQuickFix != null && (message.myLinkText.length() > 0 || message.myAfterLinkText.length() > 0)) || (message.myAdditionalFixes != null && message.myAdditionalFixes.size()
						> 0))
				{
